...
from config import BG_IMG, TILE_SIZE, WIDTH, HEIGHT, FPS
from sprites import Tile
# Импортируем функцию загрузки изображений
from utils import Camera, load_image, terminate
...
def start_game(level_filename, game_over):
    ...
    """
    player, level, level_sprites, total_coins = generate_level(
        level_filename, all_sprites, tiles_group, player_group)
    pygame.mixer.music.load('music/abyss.mp3')
    pygame.mixer.music.set_volume(0.3)
    pygame.mixer.music.play()
    bg = pygame.transform.scale(load_image(BG_IMG), (2*len(level) * TILE_SIZE, 2*len(
    ...
    while running:  # Основной игровой цикл
        ...
        # Перерисовываем фон, чтобы скрыть следы от предыдущего положения игрока (можно оптимизировать)
        screen.blit(bg, (0, 0))
        for sprite in all_sprites:
